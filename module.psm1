...
Function Get-AzureFileServiceProperties
{
    ...
    $FileUriBld.Query = "restype=service&comp=properties"
    $RequestParams=@{
        Uri=$FileUriBld.Uri;
        Method='GET';
        ExpandProperty='StorageServiceProperties'
    ...
    $RequestParams.Add('Headers',$FileHeaders)
    $FileResult=InvokeAzureStorageRequest @RequestParams
...
Function Get-AzureFileServiceChildItem
{
    ...
    PROCESS
    {
        ...
        $FileUriBld.Query="restype=directory&comp=list"
        $TokenParams=@{
            Verb='GET';
            Resource=$FileUriBld.Uri;
            Headers=@{
        ...
        Write-Verbose "[Get-AzureFileServiceChildItem] Creating SAS Token for $($FileUriBld.Uri)"
        $SasToken=New-SharedKeySignature  @TokenParams
        $FileParams=@{
            Uri=$FileUriBld.Uri;
            Method='Get';
        ...
        $TokenParams.Headers.Keys|ForEach-Object{$FileParams.Headers[$_]=$TokenParams.Headers[$_]}
        $FileParams.Headers.Add("Authorization","SharedKey $($StorageAccountName):$($SasToken)")
        $FileResult=InvokeAzureStorageRequest @FileParams
        if($FileResult.Entries -ne $null)
        {
            $DirectoryNames=@()
            foreach ($DirEntry in $FileResult.Entries.Directory)
            {
            ...
            foreach ($FileEntry in $FileResult.Entries.File)
            {
            ...
            $HasMore=-not [String]::IsNullOrEmpty($FileResult.NextMarker)
            while ($HasMore)
            {
                #Set the marker in the URI
                Write-Verbose "[Get-AzureFileServiceChildItem] Next set available @ $($FileUriBld.Uri)"
                $FileUriBld.Query="restype=directory&comp=list&marker=$($FileResult.NextMarker)"
                $FileParams.Uri=$FileUriBld.Uri
                $TokenParams.Resource=$FileUriBld.Uri
                Write-Verbose "[Get-AzureFileServiceChildItem] Creating SAS Token for $($FileUriBld.Uri)"
                $SasToken=New-SharedKeySignature  @TokenParams        
                $FileParams.Headers['Authorization']="SharedKey $($StorageAccountName):$($SasToken)"
                $FileResult=InvokeAzureStorageRequest @FileParams
                if($FileResult.Entries -ne $null)
                {
                    foreach ($DirEntry in $FileResult.Entries.Directory)
            ...
            if($Recurse.IsPresent)
            {
                ...
                {
                    $ChildPath="$($Path.TrimEnd('/'))/$($DirectoryName)"
                    Write-Verbose "[Get-AzureFileServiceChildItem] Recursing $ChildPath"
                    $ChildParams=@{
                        ...
                        AccessKey=$AccessKey;
                        Path=$ChildPath;
                        UseHttp=$UseHttp.IsPresent;
                        Recurse=$Recurse.IsPresent;
                    }
                    $SubResult=Get-AzureFileServiceChildItem @ChildParams
...
Function Get-AzureFileServiceShareStats
{
    ...
    $FileUriBld=New-Object System.UriBuilder($FileUri)
    $FileUriBld.Path=$ShareName
    $FileUriBld.Query = "restype=share&comp=stats"
    $FileHeaders = [ordered]@{
        'x-ms-date'=[DateTime]::UtcNow.ToString('R');
        "x-ms-version" = $ApiVersion;
    }
...
Function Send-AzureFileServiceFile
{
    ...
    PROCESS
    {
        foreach ($item in $InputObject)
        {
            ...
            if($CalculateChecksum.IsPresent)
            {
                Write-Verbose "[Send-AzureFileServiceFile] Calculating MD5 Hash for $($item.Name)"
                $Checksum=$item|GetMd5Hash  
                Write-Verbose "[Send-AzureFileServiceFile] Calculated Hash $Checksum"
                $FileHeaders.Add('x-ms-content-md5',$Checksum)
                $TokenParams.Add('ContentMD5',$Checksum)
            }

            $SasToken=New-SharedKeySignature @TokenParams
            Write-Verbose "[Send-AzureFileServiceFile] Creating File $($FileUriBld.Path)"
            Write-Progress -Activity "Creating File Service File" -Status "Creating $($item.Name) @ $($FileUriBld.Uri) of size $($item.Length/1MB) MB"           
            $FileHeaders.Add("Authorization","SharedKey $($StorageAccountName):$SasToken")
            ...
            $Response=InvokeAzureStorageRequest @CreateParams
            Write-Verbose "[Send-AzureFileServiceFile] Created File $($FileUriBld.Path) $($Response.Headers)"
            $InputStream=[System.IO.Stream]::Null
            $Stopwatch=New-Object System.Diagnostics.Stopwatch
            try
            {
                $BytesWritten=0
                Write-Verbose "[Send-AzureFileServiceFile] Appending File Stream  @ $($FileUriBld.Uri) - Page Size:$($BufferSize/1MB) MB"
                $Buffer=New-Object System.Byte[]($BufferSize)
                $InputStream=$item.OpenRead()
                $BytesRead=$InputStream.Read($Buffer,$BytesWritten,$BufferSize)
                while ($BytesRead -gt 0)
                {
                    $Stopwatch.Start()
                    $RangeStart=$BytesWritten
                    ...
                    $PutPageResult=Set-AzureFileServiceFileRange @PutParams                   
                    $Speed=[Math]::Round($BytesWritten/1MB/$Stopwatch.Elapsed.TotalSeconds,2)
                    $Stopwatch.Stop()
                    Write-Verbose "[Send-AzureFileServiceFile] ETag:$($PutPageResult.ETag) Transfer-Encoding:$($PutPageResult.'Transfer-Encoding') Request Id:$($PutPageResult.'x-ms-request-id')"
                    Write-Progress -Activity "Creating File Service File" -Status "Updating $($FileUriBld.Uri) $([Math]::Round($BytesWritten/1MB)) MB written - ($Speed mb/s)" -PercentComplete $($BytesWritten/$item.Length * 100)
            ...
            {
                $Stopwatch.Stop()
                Write-Progress -Activity "Creating File Service File" -Completed
                if($InputStream -ne $null)
                {
                    $InputStream.Dispose()
                }
